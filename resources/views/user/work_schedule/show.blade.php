    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="row pull-left">
                <div class="col-md-1"></div>
                <div class="col-md-2">
                <form method="GET" action="{{ route('user.weeklyreport.create') }}">
                    <button type="submit" class="btn btn-success btn-lg pull-right">週報作成</button>
                </form>
                </div>

                {{-- 月移動 --}}
                <div class="col-md-1">
                    <form method="GET" action="{{ route('user.workschedule.show') }}/{{ $lastMonthDate }}">
                        <button type="submit" class="btn btn-warning pull-right">先月</button>
                    </form>
                    <form method="GET" action="{{ route('user.workschedule.show') }}/{{ $nextMonthDate }}">
                        <button type="submit" class="btn btn-warning pull-right">翌月</button>
                    </form>
                </div>
            </div>
            ...
            <div class="row pull-right">
                <div class="col-md-1"></div>
                <div class="col-md-2">
                    <form method="POST" action="{{ route('user.workschedule.submit') }}">
                        @csrf
                        <input name="userId" type="hidden" value="{{ $user['id'] }}">
                        <input name="yearMonth" type="hidden" value="{{ $yearMonth }}">
                        <input name="date" type="hidden" value="{{ $dateToday }}">
                        <button id="submit-workschedule" type="submit" class="btn btn-lg btn-success"> 勤務表提出</button>
                    </form>
                </div>
                <div class="col-md-6">
                    <div id="submitbutton-message"></div>
                </div>
            </div>

            {{-- 勤務表登録 --}}
            {{-- ここは、後々JavaScriptなどを利用して作る
                参考　https://qiita.com/naga3/items/b37316150c1dc45b5688 --}}
            <form method="POST" action="{{ route('user.workschedule.store') }}">
                @csrf
                <div class="row pull-right">
                    <div class="col-md-1"></div>
                    <div class="col-md-2">
                        <button id="store-workschedule" type="submit" class="btn btn-primary"> 勤務表登録</button>
                    </div>
                </div>
                ...
                <table class="table table-bordered table-hover table-sm">
                    <thead>
                        <tr class="table-info">
                            ...
                            <th width="6%">
                            <select id="projectWorktime-{{ $i }}" class="form-control projectWorktimeTitle" name="projectIds[{{ $i }}]">
                    ...
                    <tbody>
                        ...
                        @endif
                            ...
                            <td width="2%"><input id="is_paid_holiday-{{ $workSchedulekey }}" name="workschedules[{{ $workSchedulekey }}][is_paid_holiday]" type="checkbox" value="1" <?php if ($workSchedule['is_paid_holiday'] === 1) echo 'checked="checked"'; ?> class="form-control is_paid_holiday"></td>
                            <td width="5%"><div style="display:flex;"><input id="starttime_hh-{{ $workSchedulekey }}" name="workschedules[{{ $workSchedulekey }}][starttime_hh]" type="number" min="0" max="30" maxLength="2" value="{{ $workSchedule['starttime_hh'] }}" class="form-control starttime_hh cul-worktime" style="width:60px;" onkeyup="nextfeild(this)">：<input id="starttime_mm-{{ $workSchedulekey }}" name="workschedules[{{ $workSchedulekey }}][starttime_mm]" type="number" min="0" max="45" step="15" maxLength="2" value="{{ $workSchedule['starttime_mm'] }}" class="form-control starttime_mm cul-worktime" style="width:60px;" onkeyup="nextfeild(this)"></div></td>
                            <td width="5%"><div style="display:flex;"><input id="endtime_hh-{{ $workSchedulekey }}" name="workschedules[{{ $workSchedulekey }}][endtime_hh]" type="number" min="0" max="30" maxLength="2" value="{{ $workSchedule['endtime_hh'] }}" class="form-control endtime_hh cul-worktime" style="width:60px;" onkeyup="nextfeild(this)">：<input id="endtime_mm-{{ $workSchedulekey }}" name="workschedules[{{ $workSchedulekey }}][endtime_mm]" type="number" min="0" max="45" step="15" maxLength="2" value="{{ $workSchedule['endtime_mm'] }}" class="form-control endtime_mm cul-worktime" style="width:60px;" onkeyup="nextfeild(this)"></div></td>
                            <td width="5%"><input id="breaktime-{{ $workSchedulekey }}" name="workschedules[{{ $workSchedulekey }}][breaktime]" type="number" step="0.25" value="{{ $workSchedule['breaktime'] }}" class="form-control breaktime cul-worktime" style="width:70px;"></td>
                            <td width="5%"><input id="breaktime_midnight-{{ $workSchedulekey }}" name="workschedules[{{ $workSchedulekey }}][breaktime_midnight]" type="number" step="0.25" value="{{ $workSchedule['breaktime_midnight'] }}" class="form-control breaktime_midnight cul-worktime" style="width:70px;"></td>
                            ...
                            <td width="6%"><input id="worktime-{{ $workSchedulekey }}-{{ $projectworkkey }}" name="workschedules[{{ $workSchedulekey }}][worktime][{{ $projectworkkey }}]" type="number" step="0.25" min="0" value="{{ $projectwork['worktime'] }}" class="form-control projecttimeaday-{{ $workSchedulekey }} worktime-{{ $projectworkkey }} cul-worktime"></td>
                            @endforeach
                            <td width="22%" id="detail-body-{{ $workSchedulekey }}"><textarea id="detail-{{ $workSchedulekey }}" name="workschedules[{{ $workSchedulekey }}][detail]" rows="1" cols="40" class="form-control detail">{!! nl2br(e($workSchedule['detail'])) !!}</textarea></td>
                        </tr>
                        @endforeach
                        {{-- 合計数を出す --}}
...
@section('customJS')
{{-- 変数初期化 --}}
const workScheduleCount = {{ count($workSchedulesJSON['workSchedules']) }};
const workingdaysByMonth = {{ $workSchedulesJSON['workingDay'] }};
...
function culcurateSummary() {
    ...
    if (worktimeSumMonthForDisplay < projecttimeSumMonthForDisplay) {
        ...
        $('#submit-workschedule').prop("disabled", true);
        $('#submitbutton-message').text('勤務時間とプロジェクト時間の合計数が一致していないと登録できません。');
        $('#submitbutton-message').css("color","red");
        ...
        $("#submit-workschedule").prop("disabled", false);
    }

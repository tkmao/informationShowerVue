...
class WorkScheduleRepository implements WorkScheduleRepositoryInterface
{
    ...
    {
        try {
            $workSchedule = $this->workSchedule->with(['projectWork.project', 'holiday'])->where('user_id', $userId)->whereBetween('workdate', [$dateFrom, $dateTo])->orderBy('workdate', 'asc')->get();
            if (!$workSchedule) {
            ...
            $workSchedulesHasId = DB::transaction(function () use ($workSchedules) {
                foreach ($workSchedules as $key => $value) {
                    ...
                    if (is_null($value['id'])) {
                        ...
                        $workSchedule->breaktime = (isset($value['breaktime']) ? $value['breaktime'] : 1);
                        $workSchedule->breaktime_midnight = (isset($value['breaktime_midnight']) ? $value['breaktime_midnight'] : 0);
                        ...
                        $update_values  = [ 'starttime_hh' => $value['starttime_hh'],
                                            ...
                                            'breaktime' => (isset($value['breaktime']) ? $value['breaktime'] : 1),
                                            'breaktime_midnight' => (isset($value['breaktime_midnight']) ? $value['breaktime_midnight'] : 0),
                                            'detail' => $value['detail'],
